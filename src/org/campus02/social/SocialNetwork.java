...
public class SocialNetwork {
	...
	public ArrayList<User> findAllFriends(User user, int level, int maxDepth){
		...
		HashSet<User> found = new HashSet<>(user.getFriends());
		for (User u: user.getFriends()){
			found.addAll(findAllFriends(u, level +1, maxDepth));
		}
	...
	public ArrayList<User> findAllfriends(User user, int level, int maxDepth){
		...
		final ArrayList<User> found = user.getFriends();
		final ArrayList<User> result = new ArrayList<>(found);
		for (final User f: found){
			final ArrayList<User> allFriends = findAllFriends(f, level+1, maxDepth);
